...
% u = [a_w(1:3, i); w_m(1:3, i)]     

%% n: process noise

%% Q: process noise covariance matrix
Qacc = eye(3) * 0.1^2;
Qrot = eye(3) * 0.002 * 300 * pi/180; % rad/s
...
while (i <= numImuMeasurements && j <= numCamMeasurements )
    ...
    if (imuTime < camTime)
        ...
        x = [x_se(1:3); quat_new./norm(quat_new); x_se(7:9)];
    ...
    orientationError(1,count) = findQuaternionError(x(4:7), q_w_i(:,i));
