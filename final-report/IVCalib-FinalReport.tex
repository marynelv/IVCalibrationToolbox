...
%% camera-IMU system.
We have implemented an IMU-camera calibration system using an unscented Kalman filter. We estimate rotation and translation between the IMU and camera, as well as the position and orientation of the IMU in world space, the velocity of the system in the world, the gravity vector, and the biases of the accelerometer and gyroscope. As input we take in points in world space with known locations (as on a target), and measurements of linear acceleration and angular velocity from the IMU.

We have tested our filter with data generated synthetically and augmented with varying amounts of noise. An extension of our filter would apply to real data as well, in which case we track points in the camera images. 
